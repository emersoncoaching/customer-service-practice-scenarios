(function () {
  const scenarios = window.CUSTOMER_SERVICE_SCENARIOS || [];
  const config = window.CUSTOMER_SERVICE_CONFIG || {};
  const app = document.querySelector("#app");
  const statusPill = document.querySelector("#status-pill");
  const draftKey = "customer-service-scenarios-draft-v1";
  const privateAccessKey = "customer-service-private-access-v1";
      ...
      <section class="start-layout">
        <form class="panel start-panel" id="start-form">
          <div class="start-heading">
            <h2>Give feedback on three short scenarios</h2>
            ${configWarning()}
          </div>
  ...
  async function submitResponses() {
    ...
    try {
      const responses = {
        scenarioVersion: "2026-07-customer-service-v1",
  ...
  async function submitToSupabase(responses) {
    const { data, error } = await supabaseClient.rpc("submit_customer_service_scenario", {
    ...
    app.innerHTML = `
      <section class="admin-grid">
        <div class="intro-copy">
          ...
          <h2>Customer Service scenario responses</h2>
  ...
  async function fetchApplicantSubmission(token) {
    const { data, error } = await supabaseClient.rpc("get_customer_service_submission_for_applicant", {
  ...
  async function fetchReviewSubmission(token) {
    const { data, error } = await supabaseClient.rpc("get_customer_service_submission_for_review", {
  ...
  async function fetchAdminSubmissions(token) {
    const { data, error } = await supabaseClient.rpc("get_customer_service_submissions_for_admin", {
      p_admin_token: token,
      p_limit: 100,
    });
    if (error) throw error;
  ...
  function adminErrorMessage(error) {
    const message = String(error && error.message ? error.message : error || "");
    if (message.includes("get_customer_service_submissions_for_admin") || message.includes("Could not find the function")) {
